...
function imageFor(el){const title=cleanTitle(el),event=(window.EVENTS||[]).find(e=>e.title===title);return event?.image||IMAGES[title]||''}
function apply(el){
  if(!el||el.dataset.imageCheck)return;const url=imageFor(el);if(!url){el.dataset.imageCheck='none';return}el.dataset.imageCheck='1';
  const img=new Image();img.decoding='async';img.referrerPolicy='no-referrer';
  img.onload=()=>{if(el.classList.contains('card')){const visual=el.querySelector('.card-visual');if(!visual)return;visual.style.backgroundImage=`linear-gradient(180deg,rgba(5,7,11,.08),rgba(5,7,11,.50)),url("${safeUrl(url)}")`;visual.style.backgroundSize='cover';visual.style.backgroundPosition='center';visual.querySelector('.emoji')?.style.setProperty('display','none');el.classList.add('has-event-image')}else{el.style.backgroundImage=`linear-gradient(180deg,rgba(5,7,11,.05),rgba(5,7,11,.62)),url("${safeUrl(url)}")`;el.style.backgroundSize='cover';el.style.backgroundPosition='center';el.querySelector('.top-emoji')?.style.setProperty('display','none');el.classList.add('has-event-image')}};
  img.onerror=()=>{el.dataset.imageCheck='failed'};img.src=url;
}
const observer='IntersectionObserver'in window?new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){apply(entry.target);observer.unobserve(entry.target)}}),{rootMargin:'240px 0px'}):null;
function scan(){document.querySelectorAll('.card,.top-card').forEach(el=>{if(el.dataset.imageCheck)return;observer?observer.observe(el):apply(el)})}
