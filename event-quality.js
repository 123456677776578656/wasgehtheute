...
function checkedAt(e){const h=health.events?.[id(e)];if(h)return h.verification_status==='verified'&&h.checked_at?h.checked_at:'';return e?.checked_at||e?.verified_at||''}
function attemptedAt(e){const h=health.events?.[id(e)];return h?.attempted_at||health.attempted_at||''}
function verificationStatus(e){const h=health.events?.[id(e)];if(!h)return'inconclusive';return h.verification_status||(h.source_ok===true&&h.checked_at?'verified':'inconclusive')}
function isRecommendable(e){const h=health.events?.[id(e)];return h?.recommendable===true}
...
function statusInfo(e){const h=health.events?.[id(e)]||{};if(h.possible_cancelled||explicitStatus(e)==='cancelled')return{code:'cancelled',label:'Abgesagt',public:false};if(h.possible_changed||explicitStatus(e)==='postponed')return{code:'postponed',label:'Verschoben',public:false};if(explicitStatus(e)==='changed')return{code:'changed',label:'Termin geändert',public:false};if(h.possible_sold_out||explicitStatus(e)==='sold-out')return{code:'sold-out',label:'Ausverkauft',public:h.recommendable===true};if(h.verification_status!=='verified'||h.recommendable!==true)return{code:'unverified',label:'Zur Nachprüfung',public:false};return{code:'confirmed',label:'Bestätigt',public:true}}
function normalise(e){const x={...e};x.title=String(x.title||'').trim();x.city=String(x.city||'').trim();x.region=String(x.region||'').trim();x.start=String(x.start||'').slice(0,10);x.end=String(x.end||x.start||'').slice(0,10);x.time=String(x.time||'').trim();x.venue=String(x.venue||x.location||'').trim();x.cats=Array.isArray(x.cats)?[...new Set(x.cats.filter(Boolean))]:[];const status=statusInfo(x);x.status=status.code;x.status_label=status.label;x.checked_at=checkedAt(x);x.check_attempted_at=attemptedAt(x);x.verification_status=verificationStatus(x);x.is_recommendable=isRecommendable(x);x.stable_id=stableId(x);const h=health.events?.[id(x)]||{};if(x.status==='cancelled'||h.possible_cancelled){x.quality_status='cancelled-warning';x.quality_note=h.reason||'Event ist als abgesagt markiert.'}else if(h.possible_changed||['changed','postponed'].includes(x.status)){x.quality_status='changed-warning';x.quality_note=h.reason||'Terminänderung erkannt – Originalquelle prüfen.'}else if(x.verification_status!=='verified'||h.occurrence_ambiguous){x.quality_status='unverified-warning';x.quality_note=h.reason||'Event und Termin konnten aktuell nicht eindeutig verifiziert werden.'}else{x.quality_status='source-ok'}return x}
...
window.EVENTS=clean;
window.WGH_EVENT_UTILS={normText,slug,id,stableId,fuzzyMatch,checkedAt,attemptedAt,verificationStatus,isRecommendable,statusInfo,sourceRank,completeness,titleSimilarity};
